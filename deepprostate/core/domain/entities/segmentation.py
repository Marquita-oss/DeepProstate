...
class SegmentationType(Enum):
    MANUAL = "manual"
    AUTOMATIC = "automatic"
    AUTOMATIC_AI = "automatic_ai"  # AI-generated segmentation
...
class SegmentationMetrics:
    # Volume metrics
    volume_voxels: int  # Total number of voxels in segmentation
    volume_percentage: float  # Percentage of total volume
    physical_volume_mm3: Optional[float] = None  # Physical volume in mm³

    # Shape metrics (optional, for compatibility)
    ...
    # Statistics
    intensity_statistics: Optional['IntensityStatistics'] = None
    ...
    def volume_mm3(self) -> float:
        return self.physical_volume_mm3 if self.physical_volume_mm3 is not None else 0.0

    def get_equivalent_sphere_diameter(self) -> float:
        volume = self.physical_volume_mm3 if self.physical_volume_mm3 is not None else 0.0
        if volume <= 0:
            return 0.0
...
class MedicalSegmentation:
    ...
    def calculate_metrics(self, spacing: 'ImageSpacing') -> SegmentationMetrics:
        ...
        total_volume = int(np.prod(self._mask_data.shape))
        volume_percentage = (self.voxel_count / total_volume * 100) if total_volume > 0 else 0.0

        self._cached_metrics = SegmentationMetrics(
            volume_voxels=self.voxel_count,
