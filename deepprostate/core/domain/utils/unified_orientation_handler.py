...
class UnifiedOrientationHandler:
    ...
    def normalize_to_ras(
        ...
        force_3d: bool = True,
        apply_physical_reorientation: bool = False
    ) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
        Load and optionally reorient medical image.

        Args:
            image_path: Path to image file
            force_3d: Force 2D images to 3D (single slice)
            apply_physical_reorientation: If True, physically reorient pixels to RAS.
                                          If False, only preserve orientation metadata.
        """
        ...
        self.logger.info(f"Loading {file_format} image: {image_path.name} (physical_reorient={apply_physical_reorientation})")

        if file_format in ['dicom', 'nifti', 'itk']:
            return self._normalize_medical_image(image_path, file_format, force_3d, apply_physical_reorientation)
    ...
    def _normalize_medical_image(
        ...
        file_format: str,
        force_3d: bool,
        apply_physical_reorientation: bool
    ) -> Tuple[np.ndarray, Dict[str, Any]]:
        if not HAS_SITK:
        ...
        try:
            ...
            if apply_physical_reorientation:
                sitk_image = self._ensure_ras_orientation(sitk_image)
                self.logger.debug("Applied physical reorientation to RAS")
            else:
                self.logger.debug("Skipped physical reorientation (preserving original orientation)")
            ...
            # Store whether physical reorientation was applied
            metadata['physical_reorientation_applied'] = apply_physical_reorientation

            self.logger.info(f"Loaded image: {numpy_array.shape}, spacing: {metadata.get('spacing', 'unknown')}")
    ...
    def normalize_mask_to_ras(
        ...
        """Load mask with optional physical reorientation."""
        mask_array, mask_metadata = self.normalize_to_ras(
            mask_path,
            force_3d=True,
            apply_physical_reorientation=apply_physical_reorientation
        )
