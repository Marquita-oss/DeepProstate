...
class AIAnalysisPersistenceService:
    def __init__(self, storage_path: Path = None):
        ...
        # Don't create directory - this service is not used (no UI to display history)
    ...
    def add_prediction_to_history(self, analysis_result: AIAnalysisResult) -> None:
        try:
            # Extract analysis_id from processing_metadata
            analysis_id = analysis_result.processing_metadata.get("analysis_id", "unknown")

            # Extract patient context from processing_metadata if available
            patient_context = analysis_result.processing_metadata.get("patient_context", {})
            ...
            history_record = {
                ...
                "series_description": patient_context.get("series_description", ""),
                "segmentation_count": len(analysis_result.segmentations),
                "overlay_count": len(analysis_result.overlay_data),
                "processing_time_ms": int(analysis_result.processing_time_seconds * 1000),
                ...
                "model_info": analysis_result.processing_metadata.get("model_predictions", {}),
                "session_id": analysis_result.processing_metadata.get("session_id", "")
            }

            self._prediction_history.append(history_record)

            self._current_session_results[analysis_id] = analysis_result

            self._save_prediction_history()

            self._logger.debug(f"Added prediction to history: {analysis_id}")
