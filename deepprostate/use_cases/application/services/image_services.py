...
class ImageLoadingService:    
    ...
    async def _validate_image_integrity(self, image: MedicalImage) -> None:
        ...
        # PERFORMANCE FIX: Only compute min/max for validation instead of full statistics
        # Full statistics (especially median) takes 2-5s for large volumes
        # This validation only needs min/max which is much faster (~100-200ms)
        import numpy as np
        image_min = float(np.min(image.image_data))
        image_max = float(np.max(image.image_data))
