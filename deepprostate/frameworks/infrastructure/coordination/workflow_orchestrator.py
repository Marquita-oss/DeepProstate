        ...
        try:
            if result.status == LoadingStatus.COMPLETED and result.data:
                if self._orchestrator._validate_medical_image_original(result.data):
                    ...
                    QTimer.singleShot(50, lambda: self._orchestrator._dialog_manager.close_progress_dialog())  # PERFORMANCE FIX: Reduced from 500ms
                else:
...
class WorkflowOrchestrator(QObject):
    ...
    def _start_loading_series(self, workflow_id: str, series_uid: str) -> None:
        try:
            ...
            QTimer.singleShot(100, lambda: self._memory_manager.force_cleanup())  # PERFORMANCE FIX: Reduced from 2000ms
    ...
    async def _complete_image_loading_original(self, workflow_id: str, file_path: str) -> None:
        try:
            ...
            QTimer.singleShot(50, lambda: self._dialog_manager.close_progress_dialog())  # PERFORMANCE FIX: Reduced from 500ms
