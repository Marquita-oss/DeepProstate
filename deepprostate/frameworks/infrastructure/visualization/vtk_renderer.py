...
# CRITICAL: Configure VTK environment BEFORE importing vtk
...
class MedicalVTKRenderer:
    def __init__(self, width: int = 800, height: int = 600):
        ...
        # CRITICAL: Prevent VTK window from showing automatically
        self._render_window.SetOffScreenRendering(1)  # Enable off-screen rendering

        # CRITICAL: Additional flags to prevent window creation
        self._render_window.SetMultiSamples(0)  # Disable antialiasing (can trigger window)
        self._render_window.SetAlphaBitPlanes(0)  # Disable alpha planes
        self._render_window.SetShowWindow(False)  # Explicitly disable window display

        # CRITICAL: Do NOT create interactor during init - it can create a window!
        # Only create it lazily when actually needed for interaction
        self._render_window_interactor = None

        with QMutexLocker(self._vtk_mutex):
            self._render_window.AddRenderer(self._renderer)
            self._render_window.SetSize(width, height)
            # Do NOT call SetRenderWindow on interactor since it's None
    ...
    def start_interaction(self) -> None:
        # Lazy create interactor only when actually needed for interaction
        if self._render_window_interactor is None:
    ...
    def _setup_medical_interaction_style(self) -> None:
        # Skip if no interactor (off-screen rendering mode)
        if self._render_window_interactor is None:
            return
