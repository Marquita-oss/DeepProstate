...
from PyQt6.QtGui import QPixmap, QFont, QPainter, QColor, QBrush, QPen, QScreen
...
class MedicalSplashScreen(QSplashScreen):
    ...
    def _calculate_adaptive_splash_size(self) -> tuple[int, int]:
        """
        Calculate adaptive splash screen size based on screen resolution.

        Returns:
            Tuple of (width, height) for splash screen
        """
        try:
            # Get primary screen
            screen = QApplication.primaryScreen()
            if screen is None:
                logger.warning("Could not detect primary screen, using default splash size")
                return (600, 400)
            ...
            logger.info(f"Splash screen: Detected screen {screen_width}x{screen_height}")

            # Calculate splash size based on screen (smaller percentage ~25-30% of screen)
            # Maintain 3:2 aspect ratio (width:height)
            splash_width = int(screen_width * 0.30)
            splash_height = int(splash_width * 2 / 3)  # Maintain 3:2 ratio

            # Ensure minimum size for readability
            splash_width = max(splash_width, 500)
            splash_height = max(splash_height, 333)

            # Ensure maximum size (don't make it too large on big screens)
            splash_width = min(splash_width, 800)
            splash_height = min(splash_height, 533)

            logger.info(f"Calculated splash size: {splash_width}x{splash_height}")

            return (splash_width, splash_height)

        except Exception as e:
            logger.error(f"Error calculating adaptive splash size: {e}")
    ...
    def _create_medical_splash_pixmap(self,
        ...
        # Calculate adaptive splash screen size based on screen resolution
        width, height = self._calculate_adaptive_splash_size()
