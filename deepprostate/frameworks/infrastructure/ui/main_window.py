...
from deepprostate.frameworks.infrastructure.utils.resources_path import (
    get_logo_svg_path, get_icon_path, get_dicom_storage_dir
)
...
class MedicalMainWindow(QMainWindow):
    ...
    def _create_child_widgets(self) -> SimpleUIComponents:
        ...
        # CRITICAL: Pass parent=self directly to prevent widgets from being created as windows
        components.central_viewer = ImageViewer2D(parent=self)

        components.right_panel = CollapsibleSidebar(parent=self)
        components.segmentation_panel = components.right_panel

        dicom_repo = DICOMImageRepository(str(get_dicom_storage_dir()))
        components.left_panel = PatientBrowserPanel(dicom_repo, parent=self)
    ...
    def _setup_collapsible_sidebar(self, sidebar: CollapsibleSidebar, patient_browser, components):
        ...
        # CRITICAL: Pass parent to prevent ghost window
        ai_panel = AIAnalysisPanel(ai_orchestrator, dynamic_config, parent=self)
        ...
        ai_panel.overlay_opacity_changed.connect(self._on_ai_overlay_opacity_changed)

        from deepprostate.frameworks.infrastructure.ui.widgets.manual_editing_panel import ManualEditingPanel
        manual_panel = ManualEditingPanel(parent=self)

        from deepprostate.frameworks.infrastructure.ui.widgets.quantitative_analysis_panel import QuantitativeAnalysisPanel
        quant_panel = QuantitativeAnalysisPanel(parent=self)

        from deepprostate.frameworks.infrastructure.ui.widgets.image_information_panel import ImageInformationPanel
        image_info_panel = ImageInformationPanel(parent=self)

        sidebar.image_information_panel = image_info_panel

        sidebar.add_panel("ai_analysis", "AI Analysis", ai_panel, QIcon(str(get_icon_path("ai_analysis.svg"))))
        sidebar.add_panel("manual_editing", "Manual Editing", manual_panel, QIcon(str(get_icon_path("manual_editing.svg"))))
        sidebar.add_panel("quantitative", "Quantitative Analysis", quant_panel, QIcon(str(get_icon_path("quantitative_analysis.svg"))))
        sidebar.add_panel("image_info", "Image Information", image_info_panel, QIcon(str(get_icon_path("image_information.svg"))))
    ...
    def _setup_main_window(self) -> None:
        self.setWindowTitle("DeepProstate")

        # Calculate adaptive window size based on screen resolution
        window_geometry = self._calculate_adaptive_window_size()

        # Set minimum size (reasonable for medical imaging application)
    ...
    def _calculate_adaptive_window_size(self) -> QRect:
        """
        Calculate adaptive window size based on available screen resolution.

        Returns:
            QRect with calculated window geometry (x, y, width, height)
        """
        try:
            # Get primary screen
            screen = QApplication.primaryScreen()
            if screen is None:
                self._logger.warning("Could not detect primary screen, using default size")
                return QRect(100, 100, 1600, 1000)
            ...
            self._logger.info(f"Detected screen resolution: {screen_width}x{screen_height}")

            # Calculate window size as percentage of screen (85% for comfortable viewing)
            ...
            self._logger.info(f"Calculated window size: {window_width}x{window_height} at position ({x}, {y})")

            return QRect(x, y, window_width, window_height)

        except Exception as e:
            self._logger.error(f"Error calculating adaptive window size: {e}")
            # Fallback to default size
            return QRect(100, 100, 1600, 1000)

    def _center_dialog_on_parent(self, dialog: QWidget) -> None:
        """
        Center a dialog window on its parent window.

        Args:
            dialog: The dialog widget to center
        """
        try:
            if not self or not dialog:
                return

            # Get parent (main window) geometry
            ...
            # Move dialog to centered position
            dialog.move(x, y)

        except Exception as e:
            self._logger.error(f"Error centering dialog: {e}")
    ...
    def _on_show_about(self) -> None:
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("About DeepProstate")
        ...
        import os
        logo_path = str(get_logo_svg_path())
        ...
        <h2 style='margin: 0 0 10px 0; color: #2980b9; font-size: 18pt;'>DeepProstate</h2>
    ...
    def _on_ai_overlay_opacity_changed(self, opacity: float) -> None:
        """Handle opacity changes from AI Analysis panel."""
        try:
            viewer = self._ui_components.central_viewer
            if not viewer:
                return
            ...
            # Update global opacity for all overlays
            overlay_service.set_global_opacity(opacity)

            # Refresh viewer to apply opacity changes
            if hasattr(viewer, 'update_overlays'):
                viewer.update_overlays()
            elif hasattr(viewer, 'refresh'):
                viewer.refresh()

            self._logger.debug(f"Global overlay opacity changed to: {opacity:.2f}")
