...
class AIAnalysisPanel(QWidget):
    ...
    overlay_opacity_changed = pyqtSignal(float)  # opacity (0.0-1.0)
    analysis_completed = pyqtSignal(object)      # AIAnalysisResult
    
    def __init__(self, orchestrator: AIAnalysisOrchestrator, dynamic_config_service: Optional[DynamicModelConfigService] = None, parent=None):
        ...
        # Persistence service removed - no UI to display history
    ...
    def _create_navigation_sections(self, parent_layout):
        ...
        results_section.setVisible(False)  # Hidden until analysis completes
    ...
    def _create_results_section(self) -> QWidget:
        ...
        self._summary_text.setMaximumHeight(150)  # Slightly larger for better readability
        ...
        self._results_tree.setMinimumHeight(150)
        # No maximum height - allows natural expansion with scroll
        ...
        results_layout.addWidget(self._results_tree, 1)  # Stretch factor for flexibility
        ...
        self._opacity_slider.setValue(70)  # Default 70%
        self._opacity_slider.setMinimumWidth(120)
        self._opacity_slider.setEnabled(False)  # Enabled after analysis completes
        ...
        self._opacity_value_label.setStyleSheet("color: #666; font-weight: bold;")
        global_controls_layout.addWidget(self._opacity_value_label)
    ...
    def _configure_nnunet_environment(self, models_base_path: Path) -> bool:
        """
        Configure nnUNet environment variables automatically for portable deployment.

        This method sets up the required nnUNet environment variables without requiring
        users to manually configure their system. It creates necessary dummy directories
        that nnUNet expects but doesn't actually use during inference.

        Args:
            models_base_path: Path where the user's AI models are stored
        ...
        try:
            ...
            # Configure nnUNet_results to point to the user-selected models directory
            os.environ['nnUNet_results'] = str(models_base_path.absolute())

            # Create dummy directories for nnUNet_raw and nnUNet_preprocessed
            # These are required by nnUNet's initialization code but NOT used during inference
            nnunet_config_dir = models_base_path / ".nnunet_config"
            nnunet_config_dir.mkdir(exist_ok=True)

            nnunet_raw_dir = nnunet_config_dir / "nnUNet_raw"
            nnunet_raw_dir.mkdir(exist_ok=True)

            nnunet_preprocessed_dir = nnunet_config_dir / "nnUNet_preprocessed"
            nnunet_preprocessed_dir.mkdir(exist_ok=True)

            os.environ['nnUNet_raw'] = str(nnunet_raw_dir.absolute())
            os.environ['nnUNet_preprocessed'] = str(nnunet_preprocessed_dir.absolute())

            self._logger.info("nnUNet environment configured successfully:")
            self._logger.info(f"  nnUNet_results = {os.environ['nnUNet_results']}")
            self._logger.info(f"  nnUNet_raw = {os.environ['nnUNet_raw']}")
            self._logger.info(f"  nnUNet_preprocessed = {os.environ['nnUNet_preprocessed']}")

            return True

        except Exception as e:
            self._logger.error(f"Failed to configure nnUNet environment: {e}")
            return False
        ...
        try:
            ...
            if dialog.exec():
                ...
                if selected_paths:
                    ...
                    # Configure nnUNet environment variables automatically (PORTABLE DEPLOYMENT)
                    env_configured = self._configure_nnunet_environment(selected_path)
                    if not env_configured:
                        QMessageBox.warning(
                            self,
                            "Environment Setup Warning",
                            "Failed to configure nnUNet environment variables.\n"
                            "Model loading may fail. Please check the logs for details."
                        )

                    self._load_models_button.setText("🔄 Loading...")
                    self._load_models_button.setEnabled(False)
                    self._status_label.setText("Configuring environment and validating AI models...")
    ...
    def _on_run_analysis_clicked(self):
        ...
        try:
            ...
            # Convert primary image to NIfTI format
            primary_image_path = self._nifti_converter.convert_to_nifti(
                medical_image=self._current_medical_image,
                temp_dir=self._temp_dir
            )

            # Detect and collect related sequences for multi-modal analysis
            additional_sequences = {}
            if selected_type == AIAnalysisType.CSPCA_DETECTION:
                detected_sequences = self._detect_related_sequences(self._current_medical_image)

                # Convert detected sequences to file paths
                for seq_name, seq_image in detected_sequences.items():
                    if seq_name.upper() in ["ADC", "HBV"]:
                        try:
                            seq_path = self._nifti_converter.convert_to_nifti(
                                medical_image=seq_image,
                                temp_dir=self._temp_dir
                            )
                            additional_sequences[seq_name] = seq_path
                            self._logger.info(f"Added {seq_name} sequence for multi-modal analysis")
                        except Exception as e:
                            self._logger.warning(f"Could not convert {seq_name} sequence: {e}")
            ...
            request = AIAnalysisRequest(
                ...
                apply_refinement=getattr(self, '_apply_refinement_cb', None) and self._apply_refinement_cb.isChecked(),
                save_intermediate_files=getattr(self, '_save_temp_files_cb', None) and self._save_temp_files_cb.isChecked(),
            ...
            # Emit signal for external listeners (optional)
    ...
    def _cancel_analysis(self):
        """Cancel running analysis safely."""
        if self._worker_thread and self._worker_thread.isRunning():
            self._logger.info("Cancelling analysis...")

            # Disconnect signals first to prevent stray emissions
            try:
                self._worker_thread.analysis_completed.disconnect()
                self._worker_thread.analysis_failed.disconnect()
                self._worker_thread.progress_update.disconnect()
            ...
            # Wait for graceful shutdown (1 second)
            if not self._worker_thread.wait(1000):
                # If thread doesn't respond, force termination
                self._logger.warning("Worker thread not responding, forcing termination")
        ...
        self._logger.info("Analysis cancelled successfully")
    ...
    def _on_analysis_completed(self, result: AIAnalysisResult):
        ...
        # Prediction history removed - no UI to display it
    ...
    def _on_opacity_changed(self, value: int):
        ...
        try:
            ...
            # Convert to 0.0-1.0 range for overlay service
            opacity = value / 100.0

            # Emit signal for overlay service to update all overlays
            self.overlay_opacity_changed.emit(opacity)

            self._logger.debug(f"Overlay opacity changed to {value}% ({opacity:.2f})")

        except Exception as e:
            self._logger.error(f"Error changing overlay opacity: {e}")
    ...
    def _save_masks(self):
        ...
        if not self._current_analysis_result:
            QMessageBox.warning(
                ...
                "No analysis results available to save. Please run an analysis first."
            )
            return
        ...
        # Open save dialog for directory selection
        file_dialog = QFileDialog(self)
        ...
        if file_dialog.exec() == QFileDialog.DialogCode.Accepted:
            save_dir = Path(file_dialog.selectedFiles()[0])

            # Map extension to export service format
            format_map = {
                ".nii.gz": "nifti",
                ".dcm": "dicom_seg",
                ".stl": "stl",
                ".mha": "nifti"  # MHA uses same exporter as NIfTI
            }

            export_format = format_map.get(extension, "nifti")

            try:
                # Call export service to save masks
                self._logger.info(f"Saving masks in {export_format} format to {save_dir}")

                exported_files = self._export_service.export_analysis_masks(
                    analysis_result=self._current_analysis_result,
                    format=export_format,
                    export_path=save_dir
                )

                if exported_files:
                    self._logger.info(f"Successfully saved {len(exported_files)} mask file(s)")

                    # Show success message to user
                    QMessageBox.information(
                        self,
                        "Masks Saved",
                        f"Successfully saved {len(exported_files)} mask file(s) to:\n{save_dir}\n\nFormat: {extension}"
                    )
                else:
                    self._logger.error("Export service returned no files")
                    QMessageBox.warning(
                        self,
                        "Save Failed",
                        "Failed to save masks. Check the logs for details."
                    )

            except Exception as e:
                self._logger.error(f"Error saving masks: {e}", exc_info=True)
                QMessageBox.critical(
                    self,
                    "Save Error",
                    f"An error occurred while saving masks:\n{str(e)}"
                )
    ...
    def clear_results(self):
        ...
        if hasattr(self, '_opacity_slider'):
            ...
            self._opacity_slider.setValue(70)  # Reset to default
            if hasattr(self, '_opacity_value_label'):
                self._opacity_value_label.setText("70%")
        self._save_masks_btn.setEnabled(False)
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            # Cancel any running analysis safely
            if self._worker_thread and self._worker_thread.isRunning():
                self._logger.info("Closing panel - cancelling running analysis...")
                ...
                if not self._worker_thread.wait(1000):
                    ...
                    self._logger.warning("Forcing thread termination on close")
                    self._worker_thread.terminate()
