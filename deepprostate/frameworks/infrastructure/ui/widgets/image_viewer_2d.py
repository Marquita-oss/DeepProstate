...
class MedicalImageCanvas(QLabel):
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self._is_panning and event.buttons() == Qt.MouseButton.RightButton:
            ...
            if self._rendering_service and self._is_active:
                ...
                self._last_pan_point = event.pos()
                self._update_timer.start(16)  # Deferred update for smooth interaction
    ...
    def set_measurement_mode(self, mode: str) -> None:
        ...
        self._update_timer.start(16)  # Deferred update
    ...
    def _get_active_overlay_id(self) -> Optional[str]:
        """
        Obtiene el ID of the overlay actualmente seleccionado usando estado sincronizado.
    ...
    def highlight_measurement(self, measurement_data: dict) -> None:
        ...
        self._update_timer.start(16)  # Deferred update  # Forzar redibujado
    ...
    def _save_state_before_action(self, overlay_id: str) -> None:
        """
        Guarda el estado antes of una acción si no se ha guardado ya in esta sesión.
    ...
    def _notify_segmentation_changed(self, overlay_id: str) -> None:
        """
        Notifica al panel que la segmentación ha cambiado for actualizar métricas.
...
class ImageViewer2D(QWidget):
    ...
    def _change_layout(self, layout_text: str):
        ...
        # Save current state
        active_overlay_id = getattr(self, '_active_overlay_id', None)
        ...
        # Update canvas with current image WITHOUT resetting segmentations
        if self._current_image:
            self._view_manager.set_image(self._current_image)
            series_uid = self._current_image.series_instance_uid if hasattr(self._current_image, 'series_instance_uid') else None
            ...
            if self._layout_mode == "single":
                ...
                for plane, canvas in [("axial", self._axial_canvas), ("sagittal", self._sagittal_canvas), ("coronal", self._coronal_canvas)]:
                    slice_data = self._view_manager.get_slice_data_for_plane(plane)
                    if slice_data:
                        canvas.set_image_data(slice_data['slice_data'], slice_data['image_spacing'], series_uid)

        # Re-sync overlay to new canvas and make visible
        if active_overlay_id:
            self._sync_overlay_to_all_canvas(active_overlay_id)
            self._overlay_service.set_overlay_visibility(active_overlay_id, True)
            self._update_canvas_for_plane(self._active_view_plane)
    ...
    def _on_segmentation_selection_changed(self, selection: str, source: str = "main_dropdown"):
        ...
        if selection == "None":
            ...
            # Save active overlay ID
            if active_overlay:
                self._active_overlay_id = active_overlay
                self._active_segmentation_name = selection
        ...
        # Sync manual panel selector when main dropdown changes
        if hasattr(self, '_manual_panel') and self._manual_panel:
            self._manual_panel.sync_with_image_viewer_segmentation(selection)
    ...
    def _auto_fit_if_needed(self):
        ...
        is_extremely_wide = aspect_ratio > 2.5   # Más de 2.5x más ancha que alta
        is_extremely_tall = aspect_ratio < 0.4   # Más de 2.5x más alta que ancha
        is_extreme_proportion = is_extremely_wide or is_extremely_tall
    ...
    def _switch_to_3d_view(self) -> None:
        ...
        try:
            ...
            if self._volume_viewer is None:
                ...
                self._volume_viewer = VolumeViewer3D(parent=self)
    ...
    def _switch_to_2d_view(self) -> None:
        ...
        try:
            ...
            # Re-sync overlay when returning to 2D
            if active_overlay_id:
                self._sync_overlay_to_all_canvas(active_overlay_id)
                self._overlay_service.set_overlay_visibility(active_overlay_id, True)
                self._update_canvas_for_plane(self._active_view_plane)
    ...
    def connect_manual_editing_panel(self, manual_panel) -> None:
        ...
        try:
            ...
            # Connect active segmentation changes from panel
            manual_panel.active_segmentation_changed.connect(self._on_active_segmentation_changed)

            # Connect slice changes to update panel metrics
            self.slice_changed.connect(manual_panel._on_slice_changed_from_viewer)
    ...
    def _update_panel_segmentations(self) -> None:
        """
        Actualiza el dropdown of segmentaciones in the panel manual.
        """
        try:
            ...
            # SOLUCIÓN Usar directamente el overlay service of the canvas activo
            available_overlays = []
            active_canvas = self._get_active_canvas()
    ...
    def _find_real_masks_from_debug_system(self) -> List[str]:
        """
        Busca máscaras reales of the sistema que las detecta automáticamente.
    ...
    def _sync_overlay_to_all_canvas(self, overlay_id: str) -> None:
        """
        Sincroniza el overlay_id directamente a todos los canvas.
        """
        try:
            canvases = [self._main_canvas, self._axial_canvas, self._sagittal_canvas, self._coronal_canvas]
            for canvas in canvases:
    ...
    def _on_active_segmentation_changed(self, segmentation_name: str, overlay_id: str) -> None:
        """
        Handles cambios of segmentación activa from el panel.
    ...
    def _find_manual_panel(self):
        """
        Busca el panel manual editing in la aplicación.
    ...
    def _get_manual_panel_visibility_state(self) -> bool:
        """
        Verifica el estado of the botón of visibilidad of the panel manual.
    ...
    def _sync_manual_panel_visibility_button(self, visible: bool) -> None:
        """
        Sincroniza el botón of visibilidad of the panel manual.
    ...
    def _sync_main_dropdown_selection(self, segmentation_name: str) -> None:
        """
        Sincroniza el selector principal with la selección of the panel manual.
